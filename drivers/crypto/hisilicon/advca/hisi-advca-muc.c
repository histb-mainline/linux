...
struct hica_muc_chan {
	...
	union {
		/* for channel 0 */
		struct {
			/* flatten scatterlist to avoid complexity */
			void *inout;
			/* must be power of 2 */
		...
		struct {
			...
			/* static iv buffer; concated with stream padding */
			void *iv;
...
/*
 * 0: output is ready
 * -EINPROGRESS: input submitted, call hica_muc_chan_emit() to emit request
 * -EBUSY: wait hardware consuming input
 * other: error
 */
static int
...
static void hica_muc_chan_emit(struct hica_muc_chan *chan)
{
	...
	/* clear dirty bit in interrupt handler */
}
