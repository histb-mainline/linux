...
{
	static unsigned int width[] = { 8, 16, 32 };
	static unsigned int bsize[] = { 1, 4, 8, 16, 32, 64, 128, 256 };
...
{
	struct histb_dma_desc *desc = to_histb_dma_desc(vdesc);
	struct histb_dma_chan *chan = to_histb_dma_chan(vdesc->tx.chan);

	if (desc->value)
		dma_pool_free(chan->pool, desc->value, desc->src_addr);
	kfree(desc);
}
		    ...
		    unsigned int id, struct dma_pool *pool)
{
	...
	chan->vchan.desc_free = histb_dma_desc_free;
...
histb_dma_prep_dma_memop(struct dma_chan *dmachan, dma_addr_t dst,
			 ...
			 int *value)
{
	...
	if (value)
		...
		desc->ctrl |= DMAC_CHAN_CTRL_SRC_INC;

	return vchan_tx_prep(&chan->vchan, &desc->vdesc, flags);

err:
	if (value)
		dma_pool_free(chan->pool, value, src);
	return NULL;
}
...
histb_dma_prep_dma_memcpy(struct dma_chan *dmachan, dma_addr_t dst,
			  dma_addr_t src, size_t len, unsigned long flags)
{
	return histb_dma_prep_dma_memop(dmachan, dst, src, len, flags, NULL);
}
			  ...
			  size_t len, unsigned long flags)
{
	...
	return histb_dma_prep_dma_memop(dmachan, dst, buf_addr, len, flags,
					buf);
}
...
static int histb_dma_probe(struct platform_device *pdev)
{
	...
	dmadev->copy_align = DMAENGINE_ALIGN_32_BYTES;
	dmadev->fill_align = DMAENGINE_ALIGN_32_BYTES;
	dmadev->dev = dev;
